...
class PresensiController extends Controller
{
    ...
    public function breakist(Request $request)
    {
        $kode_jam_kerja = $request->kode_jam_kerja;
        $nik = Auth::guard('karyawan')->user()->nik;
        $status_location = Auth::guard('karyawan')->user()->status_location;
        ...
        if ($status_location == 1 && $radius > $lok_kantor->radius_cabang) {
            ...
            if ($cek > 0 && $jamkerja->status_istirahat = 1) {
                if ($jam < $jamkerja->awal_jam_istirahat) {
                    echo "error|Maaf Belum Waktunya Istirahat";
                } else if (!empty($datapresensi->akhir_istirahat)) {
                    echo "error|Anda Sudah Melakukan Absen Istirahat ! |out";
                } else {
                    $data = [
                        'akhir_istirahat' => $jam,
                        'foto_break_in' => $fileName,
                        'lokasi_break_in' => $lokasi
                    ];
                    $update = DB::table('presensi')->where('tgl_presensi', $tgl_presensi)->where('nik', $nik)->update($data);
                    if ($update) {
                        echo "success|Terimakasih, Semangat Selalu !|out";
                        Storage::put($file, $image_base64);
                ...
                if ($jam < $jamkerja->awal_jam_istirahat) {
                    echo "error|Maaf Belum Waktunya Istirahat|in";
                } else if ($jam > $jamkerja->akhir_jam_istirahat) {
                    echo "error|Maaf Waktu Istirahat Sudah Habis |in";
                } else {
                    $data = [
                        'nik' => $nik,
                        'tgl_presensi' => $tgl_presensi,
                    ...
                    $simpan = DB::table('presensi')->insert($data);
                    if ($simpan) {
                        echo "success|Selamat, Istirahat!|in";
